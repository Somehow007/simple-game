...
| 功能模块 | 功能描述 | 优先级 |
|---------|---------|--------|
| 扫雷核心引擎 | 地雷布雷算法、区域展开（flood fill）、胜负判定 | P0 |
| 地图生成器 | 支持多难度等级（初级/中级/高级/自定义）的地图生成 | P0 |
| 游戏界面 | 网格交互、左右键操作、触屏长按支持 | P0 |
| 游戏状态管理 | 自动保存、继续游戏、新游戏 | P0 |
| 计时系统 | 游戏计时、暂停/恢复、最佳记录 | P1 |
| 标记系统 | 旗帜标记、问号标记、数字显示 | P1 |
| 统计系统 | 游戏历史、胜率统计、难度分布、连续记录 | P1 |
| 自定义设置 | 主题切换（亮色/暗色）、音效开关、输入模式切换 | P1 |
| 每日挑战 | 每日推送一道特定地图，全球排名 | P2 |
| 成就系统 | 解锁成就、里程碑奖励 | P2 |
| 多种变体 | 彩色数字、极限计时模式、盲扫模式 | P2 |

### 1.2 目标用户群体

- **核心用户**：逻辑推理游戏爱好者，年龄 20-65 岁，偏好数独和扫雷
- **休闲用户**：寻求日常脑力锻炼的普通用户
- **进阶用户**：追求高难度挑战和竞技排名的资深玩家
- **入门用户**：对数独或扫雷感兴趣但尚未掌握技巧的新手
...
| 任务 | 交付物 | 验收标准 |
|------|--------|---------|
| 定义数独类型系统 | types.ts | 所有核心类型定义完成，导出正确 |
...
| 实现地雷布雷算法 | generator.ts | 可生成 4 个难度等级的地图，地雷分布均匀 |
| 实现区域展开算法 | floodfill.ts | 点击空白格子时正确展开相连空白区域 |
| 实现胜负判定 | validator.ts | 正确判定踩雷胜利、清除所有安全格失败 |
| 实现计时系统 | timer.ts | 精确计时，支持暂停/恢复 |
| 实现标记逻辑 | marker.ts | 旗帜和问号标记逻辑正确 |
| 核心引擎单元测试 | __tests__/ | 测试覆盖率 ≥ 95% |
| 性能基准测试 | benchmarks/ | 生成 10000 张地图 < 1s |
...
| 任务 | 交付物 | 验收标准 |
|------|--------|---------|
| 扫雷网格组件 | MineGrid.tsx | 网格渲染正确，支持左键/右键/长按 |
| 状态显示组件 | StatusDisplay.tsx | 地雷计数、计时器、笑脸按钮 |
| 工具栏组件 | Toolbar.tsx | 新游戏/难度选择/设置 |
| 游戏状态管理 | mineStore.ts | Zustand store 完整，持久化正常 |
| 主题系统 | ThemeProvider | 亮色/暗色切换流畅 |
| 响应式布局 | CSS 布局 | 桌面/平板/手机三端适配 |
| 游戏页面 | MinePage.tsx | 完整游戏流程可运行 |
| 设置页面 | SettingsPage.tsx | 所有设置项可持久化 |
| 统计页面 | StatsPage.tsx | 数据展示正确 |
| PWA 配置 | manifest.json, SW | 可安装到桌面，离线可玩 |
| Web 版 E2E 测试 | e2e/ | 核心流程测试通过 |
...
| 菜单栏 (macOS) | menu.rs | 原生菜单栏集成（双游戏切换） |
...
2026
 ...
 │  ✓ 性能基准达标
 │
 ▼ M4: 数独 Web 版 Alpha ────────── 第 3 周
 │  ✓ 数独核心游戏流程可运行
 │  ✓ 基础 UI 组件完成
 │  ✓ 内部可体验
 │
 ▼ M5: 扫雷 Web 版 Alpha ────────── 第 5 周
 │  ✓ 扫雷核心游戏流程可运行
 │  ✓ 基础 UI 组件完成
 │  ✓ 内部可体验
 │
 ...
 │  ✓ 自动更新功能就绪
 │
 ▼ M10: iOS 版 Alpha ────────────── 第 10 周
 │  ✓ iPhone + iPad 可运行
 │  ✓ 触控交互优化完成
 │
 ▼ M11: iOS 版正式发布 ───────────── 第 11 周
 │  ✓ App Store 审核通过
 │  ✓ iCloud 同步就绪
 │
 ▼ M12: Android 版 Alpha ────────── 第 12 周
 │  ✓ 主流设备可运行
 │  ✓ Material 适配完成
 │
 ▼ M13: Android 版正式发布 ───────── 第 13 周
 │  ✓ Google Play 上架
 │  ✓ Widget 功能就绪
 │
 ▼ M14: 全平台优化发布 ──────────── 第 14 周
...
| M2 数独引擎就绪 | 第 1 周 | 数独核心引擎包 | 测试覆盖率 ≥ 95%，性能达标 |
| M3 扫雷引擎就绪 | 第 2 周 | 扫雷核心引擎包 | 测试覆盖率 ≥ 95%，性能达标 |
| M4 数独 Web Alpha | 第 3 周 | 可运行数独 Web 版 | 核心游戏流程可体验 |
| M5 扫雷 Web Alpha | 第 5 周 | 可运行扫雷 Web 版 | 核心游戏流程可体验 |
| M6 Web Beta | 第 6 周 | 功能完整 Web 版 | 全功能 + PWA + E2E 测试 |
| M7 Web 发布 | 第 7 周 | 正式 Web 版 | 上线可访问，性能达标 |
| M8 桌面 Alpha | 第 8 周 | 桌面内测版 | 双平台可运行 |
| M9 桌面发布 | 第 9 周 | 桌面正式版 | 安装包可分发 |
| M10 iOS Alpha | 第 10 周 | iOS 内测版 | TestFlight 可安装 |
| M11 iOS 发布 | 第 11 周 | iOS 正式版 | App Store 上架 |
| M12 Android Alpha | 第 12 周 | Android 内测版 | 主流设备可运行 |
| M13 Android 发布 | 第 13 周 | Android 正式版 | Google Play 上架 |
| M14 全平台优化 | 第 14 周 | 全平台最终版 | 国际化 + 监控 + 优化完成 |
...
| 扫雷组件交互 | MineGrid 点击、标记、区域展开联动 | Vitest + Testing Library |
| 数据流 | 生成→游玩→保存→恢复（双游戏） | Vitest |
...
| 场景 | 测试重点 | 工具 |
|------|---------|------|
| 数独完整游戏流程 | 新游戏→填数→完成→统计 | Playwright |
| 扫雷完整游戏流程 | 新游戏→扫雷→胜利/失败→统计 | Playwright |
...
| 数独生成时间 | < 50ms (单道) | 基准测试 |
| 扫雷地图生成时间 | < 5ms (单张) | 基准测试 |
| 内存占用 | < 100MB (Web) / < 50MB (原生) | 性能分析器 |
| 安装包体积 | < 8MB (Web) / < 12MB (Desktop) / < 25MB (Mobile) | 构建产物 |
...
| **计算优化** | Web Worker 执行地图生成/求解 | 主线程零阻塞 |
| **扫雷优化** | 增量区域展开、虚拟网格渲染 | 复杂地图流畅 60fps |
...
| **包体积优化** | Tree-shaking、动态 import、依赖分析 | 总包 < 250KB gzip |
...
| 风险 | 概率 | 影响 | 等级 | 应对策略 |
|------|------|------|------|---------|
| **数独谜题生成算法性能不达标** | 中 | 高 | 🔴 高 | 提前进行算法预研；准备回退方案（预生成谜题库）；使用 Web Worker 卸载计算 |
| **扫雷区域展开性能问题** | 中 | 中 | 🟡 中 | 优化 flood fill 算法；使用迭代而非递归；限制展开深度 |
| **双游戏代码组织混乱** | 中 | 中 | 🟡 中 | 清晰的模块边界；共享受组件抽离；严格的代码审查 |
...
| **移动端触控体验不佳** | 中 | 中 | 🟡 中 | 早期触控原型验证；参考成熟游戏 App 交互模式；A/B 测试不同交互方案 |
...
**问题**：大面积空白区域的递归展开可能导致栈溢出和性能问题。

**解决方案**：
1. 使用迭代（Iteration）而非递归（Recursion）实现 flood fill
2. 使用 BFS（广度优先搜索）算法
3. 对展开区域进行剪枝，限制最大展开范围
4. 使用虚拟网格渲染，仅渲染可视区域
5. 对极端情况进行压力测试

```typescript
// 迭代式区域展开
function floodFill(grid: Cell[][], x: number, y: number): Set<string> {
  ...
  while (queue.length > 0) {
    ...
    if (grid[cy][cx].isMine) continue;

    visited.add(key);

    if (grid[cy][cx].adjacentMines === 0) {
      // 添加相邻格子到队列
...
**问题**：需要在单个应用内管理两款游戏的状态，同时保持代码清晰。

**解决方案**：
1. 使用独立的 Zustand store 管理每款游戏
2. 共享的基础设置（主题、音效）使用单独的 store
3. 使用 React Context 提供全局状态访问
4. 统一的存储 key 前缀区分游戏数据
...
// 扫雷状态 store
const useMinesweeperStore = create<MinesweeperState>((set, get) => ({...}));
...
**问题**：扫雷网格在手机屏幕上单元格较小，触控精准选择困难。

**解决方案**：
1. 触控区域大于视觉区域（padding 扩展）
2. 支持长按标记旗帜
3. 滑动手势支持（滑动快速标记多个格子）
4. 触觉反馈 (Haptic Feedback) 确认操作
5. 提供缩放功能
...
1. 标准扫雷在矩形网格中进行，分为初级（9×9，10 颗雷）、中级（16×16，40 颗雷）、高级（30×16，99 颗雷）
2. 点击格子可揭开，揭开数字表示该格子周围 8 个方向的地雷数量
3. 点击数字格子时可同时按下鼠标右键标记旗帜
4. 长按格子可标记/取消旗帜
5. 揭开所有非地雷格子即为胜利，揭开地雷则为失败
6. 计时从第一次点击开始计时

### D. 扫雷难度等级定义

| 等级 | 网格大小 | 地雷数 | 地雷密度 | 预计完成时间 |
|------|---------|--------|---------|-------------|
| 初级 | 9×9 | 10 | 12.3% | 0.5-2 分钟 |
| 中级 | 16×16 | 40 | 15.6% | 5-15 分钟 |
| 高级 | 30×16 | 99 | 20.6% | 30-120 分钟 |
| 自定义 | 可配置 | 可配置 | 可配置 | 取决于配置 |
...
| 扫雷展开算法 | 递归 / 迭代 BFS | 迭代 BFS | 避免栈溢出，更好控制展开深度 |
